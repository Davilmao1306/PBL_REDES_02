...
def active_broker_ids() -> list[int]:
    """ Identifica os nós vivos na rede P2P. """
    now = time.time()
    ids = [BROKER_ID]
    with state_lock:
...
def coordinator_id() -> int:
    """ ELEIÇÃO DE LÍDER (Sem servidor central): O menor ID online lidera a rede. """
...
def merge_occurrence(data: dict[str, Any]) -> None:
    ...
    clock.update(incoming.lamport_ts) # Sincroniza Relógio Lógico com a rede
    with state_lock:
...
def replicate_state() -> None:
    """ PROPAGAÇÃO ANTI-ENTROPIA: Réplica o estado para todos os outros brokers. """
...
def mark_peer_down(peer_id: int) -> None:
    """ AUSÊNCIA DE PONTO ÚNICO DE FALHA: Se um setor cai, os outros assumem as suas ocorrências. """
    with state_lock:
        previous = peer_status.get(peer_id, {})
...
def check_failed_missions() -> None:
    """ TOLERÂNCIA A FALHA DE DRONE: Replaneamento caso o drone seja abatido (Barema Criterio 4). """
        ...
        for drone in list(drones.values()):
            if drone.status == DroneStatus.BUSY.value and drone.assigned_occurrence_id:
                if now - drone.last_seen > TIMEOUT_LIMITE:
                    ...
                    # FILA DISTRIBUÍDA: A missão volta a ficar pendente para o próximo drone
                    if occ and occ.status != OccurrenceStatus.DONE.value:
                        ...
                        broker_log(f"Ocorrencia {occ_id} voltou para a fila.")
...
def dispatch_once() -> None:
    """ EXCLUSÃO MÚTUA DISTRIBUÍDA: Apenas o líder agenda missões, evitando duplicidade de cobertura. """
    if time.time() - started_at < DISPATCH_START_DELAY:
        return
    if coordinator_id() != BROKER_ID:
        return # Os brokers "não-líderes" apenas replicam, não tomam decisão de alocação

    with state_lock:
        pending = sorted(
            [item for item in occurrences.values() if item.status == OccurrenceStatus.PENDING.value],
            key=lambda item: item.ordering_key, # Usa a ordenação de prioridade restrita
        )
        available = sorted(
            [
                item
        ...
        for occurrence in pending:
            if not available:
                break # Mantém a requisição na fila distribuída se não houver drones livres
            ...
            broker_log(f"reserva: drone={drone.drone_id} ocorrencia={occurrence.occurrence_id} prioridade={occurrence.severity} ts={occurrence.lamport_ts}")
            
            # ARQUITETURA PUSH: Empurra o alerta para o drone assincronamente
...
def notify_drone(drone: DroneInfo, occurrence: Occurrence) -> None:
    ...
    try:
        ...
        # Recuperação imediata se a rede falhar no momento exato de enviar a ordem PUSH
        with state_lock:
...
def register_drone(data: DroneRegistration) -> dict[str, Any]:
    """ Endpoint que recebe o PUSH dos Drones informando a sua disponibilidade/vida """
    with state_lock:
