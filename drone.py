...
state_lock = threading.RLock() # Previne condições de corrida locais
...
def register_as_available() -> bool:
    """ Informa a rede P2P que o drone está livre. """
...
def send_mission_heartbeats(stop_heartbeat: threading.Event):
    """
    HEARTBEAT EM MISSÃO (Tolerância a Falhas):
    Durante o voo, o drone empurra (PUSH) atualizações constantes para o broker.
    Se o drone for abatido, este sinal para e o broker aciona o replaneamento.
    """
    payload = {"drone_id": DRONE_ID, "callback_url": DRONE_URL}
    while not stop_heartbeat.is_set():
        for broker in broker_candidates():
            try:
                requests.post(f"{broker}/drones/register", json=payload, timeout=1)
                break
            except requests.RequestException:
                continue
        time.sleep(2)
...
def finish_mission(occurrence_id: str, broker_url: str) -> None:
    ...
    # Inicia os sinais de vida de missão em paralelo
    stop_heartbeat = threading.Event()
    hb_thread = threading.Thread(target=send_mission_heartbeats, args=(stop_heartbeat,), daemon=True)
    hb_thread.start()

    time.sleep(duration) # Simula o voo real
...
def receive_mission(data: MissionIn) -> dict[str, str]:
    """ ARQUITETURA PUSH: O Coordenador empurra a ordem diretamente para cá. """
    global current_mission
    occurrence_id = data.occurrence["occurrence_id"]
    with state_lock:
        # Garante NÃO-DUPLICIDADE de cobertura localmente
